(function() {
    ...
    document.addEventListener('mousedown', () => cur && cur.classList.add('clicked'));
    document.addEventListener('mouseup',   () => cur && cur.classList.remove('clicked'));
    ...
    // Ne démarre les listeners qu'une fois la transition terminée
    ...
    if (window.TRANSITION_ACTIVE) {
        window.addEventListener('transitionDone', enableStartListeners, { once: true });
    } else {
        enableStartListeners();
    }

    const tabsScroll = document.getElementById('tabs-scroll');
    const btnLeft  = document.getElementById('tabs-left');
    const btnRight = document.getElementById('tabs-right');

    if (btnLeft && tabsScroll) {
    ...
    function triggerWelcomeMessage() {
        ...
        setTimeout(() => {
            ...
            setTimeout(async () => {
                ...
                // Lancement du message réel — débloque isStreaming à la fin
